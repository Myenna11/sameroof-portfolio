...
function chatPage() {
 ...
 ${state.room ? "" : `<button class="task-ribbon" data-page="tasks"><span class="task-ribbon-icon">${icon("task")}</span><span><strong>${activeTasks().length ? "有 " + activeTasks().length + " 件事，等我们一起完成" : "今天的黑板"}</strong><small>${esc(activeTasks()[0]?.title || "把心里惦记的事，轻轻钉在这里。")}</small></span>${icon("chevron", 17)}</button>`}
