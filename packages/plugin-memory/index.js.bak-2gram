...
function open(roomDir) {
  ...
  const api = {
    ...
    recall(query, n = 5) {
      ...
      const q = grams(query);
      const scored = visible(all).map(m => { const { hit, size } = overlap(q, m.content); return { m, s: hit / Math.sqrt(size + 1) * (0.5 + m.confidence) }; })
...
module.exports = { open, redact, defuse, normalize, RENDER_HEADER, MemoryError, HandAuthoredProtectedError, HumanOnlyError, NotFoundError, StateError };
