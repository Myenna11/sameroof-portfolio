...
test('追加 update 行折叠：recall 加热只追加，不重写；forget 也只追加', async () => {
  ...
  const hits = await M.recall('草莓蛋糕');
  ...
  assert.equal((await M.recall('草莓蛋糕')).length, 0);
});
...
test('supersede → approve 链：旧的不动、等人点头；非人类 approve 亲笔抛 MEM-HUMAN-ONLY', async () => {
  const M = mem.open(tmpRoom());
  ...
  assert.deepEqual((await M.recall('维护者生日')).map(x => x.id), [a.id]);
  ...
  assert.deepEqual((await M.recall('维护者生日')).map(x => x.id), [b.id]);
...
test('同 fact_key 自动 under_review；recall/recent 只回 current，不回 under_review/superseded/discarded', async () => {
  const M = mem.open(tmpRoom());
...
test('merge / discard', async () => {
  ...
  assert.deepEqual((await M.recall('维护者 美式')).map(x => x.id), [m.id]);
  ...
  assert.equal((await M.recall('垃圾信息')).length, 0);
});
...
test('compact 后内容一致，行数变少，旧字段清掉', async () => {
  const room = tmpRoom(); fs.mkdirSync(path.join(room, 'memory'));
  ...
  await M.recall('下雨天'); M.approve(b.id, { by: 'h1', species: 'human' }); M.forget('旧行');
