...
function think(system, user) {
  const body = JSON.stringify({ model: R.room.model.id, messages: [{ role: 'system', content: system }, { role: 'user', content: user }], stream: false, max_tokens: 1500, thinking: { type: 'enabled', effort: 'low' } });
